    ...
    public partial class AllReports : Form
    {
        ...
        private void deleteToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            //if (dgvSaleInvoices.SelectedRows.Count == 1)
            {
                ...
                {
                    MainClass.con.Open();
                    cmd = new SqlCommand("select s.SalesID,p.Pcode,u.UnitName,sd.SalesQty,u.UnitID,s.InvoiceNo from Sales s inner join SalesDetails sd on sd.Sales_ID = s.SalesID inner join Products p on p.Pcode = sd.Product_ID inner join Units u on u.UnitID = sd.SalesUnit_ID where s.SalesID = '" + salesid + "'", MainClass.con);
