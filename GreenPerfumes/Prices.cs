    ...
    public partial class Prices : Form
    {
            ...
            ,string data = null)
        {
            ...
            try
            {
                ...
                if (data != null)
                {
                    cmd = new SqlCommand("select pr.ID ,p.ProductName, p.Pcode,u.UnitName, pr.PurchaseRate,pr.SaleRate from Prices pr inner join Products p on p.Pcode = pr.Pcode inner join Units u on u.UnitID = pr.Pro_Unit where p.ProductName like '%" + data + "%'   group by p.ProductName, p.Pcode,u.UnitName,pr.PurchaseRate,pr.SaleRate,pr.ID order by p.ProductName asc ", MainClass.con);
                }
                else
                {
                    cmd = new SqlCommand("select pr.ID ,p.ProductName, p.Pcode,u.UnitName, pr.PurchaseRate,pr.SaleRate from Prices pr inner join Products p on p.Pcode = pr.Pcode inner join Units u on u.UnitID = pr.Pro_Unit  group by p.ProductName, p.Pcode,u.UnitName,pr.PurchaseRate,pr.SaleRate,pr.ID order by p.ProductName asc ", MainClass.con);
                }
        ...
        private void guna2Button1_Click(object sender, EventArgs e)
        {
            ...
            if (edit == 0)
            {
                ...
                try
                {
                    ...
                    MainClass.con.Open();
                    SqlCommand cmd1 = new SqlCommand("insert into Prices (Pcode,Pro_Unit,PurchaseRate,SaleRate) values(@Pcode,@Pro_Unit,@PurchaseRate,@SaleRate)", MainClass.con);
                    cmd1.Parameters.AddWithValue("@Pcode", proID);
                    cmd1.Parameters.AddWithValue("@Pro_Unit", unitid);
                ...
                if(edit == 1)
                {
                    ...
                    else
                    {
                        try
                        {
                            ...
                            MainClass.con.Open();
                            cmd = new SqlCommand("update  Prices set Pcode = @Pcode, Pro_Unit = @Pro_Unit, PurchaseRate = @PurchaseRate, SaleRate = @SaleRate where ID = @ID", MainClass.con);
            ...
            if (dgvpricing != null)
            {
                if (dgvpricing.Rows.Count > 0)
                {
                    if (dgvpricing.SelectedRows.Count == 1)
                    {
                        ...
                        {
                            MainClass.con.Open();
                            SqlCommand cmd = new SqlCommand("delete from Prices where ID = @ID", MainClass.con);
        ...
        {
            PricesOther po = new PricesOther();
            MainClass.showWindow(po, this, MDI.ActiveForm);
        }

        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            LoadProducts(dgvpricing, ProNameGV, PcodeGVG, UnitGV, PurchaseRateGV, SaleRateGV, IDName, txtSearch.Text);
        }
