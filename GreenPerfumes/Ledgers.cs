    ...
    public partial class Ledgers : Form
    {
        ...
        private void ShowLedgerC(DataGridView dgv, DataGridViewTextBoxColumn InvoiceGV, DataGridViewTextBoxColumn NameGv, DataGridViewTextBoxColumn PhoneGV, DataGridViewTextBoxColumn TypeGV, DataGridViewTextBoxColumn DateGV, DataGridViewTextBoxColumn TotalGV, DataGridViewTextBoxColumn PaidGV, DataGridViewTextBoxColumn BalanceGV)
        {
            try
            {
                ...
                {
                    cmd = new SqlCommand("select s.InvoiceNo, p.PersonName, p.PersonPhone, cl.InvoiceType, format(cl.InvoiceDate, 'dd/MM/yyyy') as 'Date', cl.TotalAmount, cl.PaidAmount, cl.RemainingBalance from CustomerLedgers cl inner join Persons p on p.PersonID = cl.Customer_ID inner join Sales s on s.CustomerInvoice_ID = cl.CustomerInvoice_ID where cl.InvoiceType = 'Credit' and  p.PersonName = '" + cboCustomer.Text + "'  and format(cl.InvoiceDate, 'dd/MM/yyyy')  between '"+dtFrom.Value.ToShortDateString()+"' and '"+dtTo.Value.ToShortDateString() + "'  and RemainingBalance != '0'", MainClass.con);
                    changed = 0;
                }
                else if(changed == 3)
                {
                    cmd = new SqlCommand("select s.InvoiceNo, p.PersonName, p.PersonPhone, cl.InvoiceType, format(cl.InvoiceDate, 'dd/MM/yyyy') as 'Date', cl.TotalAmount, cl.PaidAmount, cl.RemainingBalance from CustomerLedgers cl inner join Persons p on p.PersonID = cl.Customer_ID inner join Sales s on s.CustomerInvoice_ID = cl.CustomerInvoice_ID where cl.InvoiceType = 'Credit'  and format(cl.InvoiceDate, 'dd/MM/yyyy')  between '" + dtFrom.Value.ToShortDateString() + "' and '"+dtTo.Value.ToShortDateString() + "'  and RemainingBalance != '0'", MainClass.con);
                    changed = 0;
                }
                else
                {
        ...
        private void ShowLedgerS(DataGridView dgv, DataGridViewTextBoxColumn InvoiceGV, DataGridViewTextBoxColumn NameGv, DataGridViewTextBoxColumn PhoneGV, DataGridViewTextBoxColumn TypeGV, DataGridViewTextBoxColumn DateGV, DataGridViewTextBoxColumn TotalGV, DataGridViewTextBoxColumn PaidGV, DataGridViewTextBoxColumn BalanceGV)
        {
            try
            {
                ...
                {
                    cmd = new SqlCommand("select s.InvoiceNo, p.PersonName, p.PersonPhone, cl.InvoiceType, format(cl.InvoiceDate, 'dd/MM/yyyy') as 'Date', cl.TotalAmount, cl.PaidAmount, cl.RemaingBalance from SupplierLedgers cl inner join Persons p on p.PersonID = cl.Supplier_ID inner join Purchases s on s.SupplierInvoice_ID = cl.SupplierInvoice_ID where cl.InvoiceType = 'Credit' where cl.InvoiceType = 'Credit' and  p.PersonName  = '" + cboSupplier.Text + "'  and format(cl.InvoiceDate, 'dd/MM/yyyy')  between '" + dtFromS.Value.ToShortDateString() + "' and '" + dtToS.Value.ToShortDateString() + "' and RemaingBalance != '0' ", MainClass.con);
                    changed = 0;
                }
                else if (changed == 3)
                {
                    cmd = new SqlCommand("select s.InvoiceNo, p.PersonName, p.PersonPhone, cl.InvoiceType, format(cl.InvoiceDate, 'dd/MM/yyyy') as 'Date', cl.TotalAmount, cl.PaidAmount, cl.RemaingBalance from SupplierLedgers cl inner join Persons p on p.PersonID = cl.Supplier_ID inner join Purchases s on s.SupplierInvoice_ID = cl.SupplierInvoice_ID where cl.InvoiceType = 'Credit'  and format(cl.InvoiceDate, 'dd/MM/yyyy')  between '" + dtFromS.Value.ToShortDateString() + "' and '" + dtToS.Value.ToShortDateString() + "' and RemaingBalance != '0'  ", MainClass.con);
                    changed = 0;
                }
                else
                {
        ...
        private void btnPay_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (lblp.Text == "Supplier")
                {
                    ...
                    cmd.Parameters.AddWithValue("@SaleLastPaid", txtPaying.Text);
